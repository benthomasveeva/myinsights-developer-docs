...
(function () {
  ...
  window.VeevaUtilities = function () {
    ...
    veevaUtil.replaceStringInWhereClause = function (
      whereClause,
      input,
      stringToBeReplaced,
    ) {
      ...
      for (i = 0; i < indices.length; i += 1) {
        if (i === 0) {
          ...
          result += whereClause.slice(
            indices[i - 1] + stringToBeReplaced.length,
            indices[i],
          );
        }
      ...
      result += whereClause.slice(
        indices[indices.length - 1] + stringToBeReplaced.length,
      );
...
(function (Q) {
  ...
  window.OnlineAPI = function () {
    ...
    olAPI.htmlReportIdAndUUIDDeferred.promise.then(
      function (htmlReportIdentifiers) {
    ...
    function delegateOnlineRequest(
      queryConfig,
      deferredId,
      deferred,
      respHandler,
    ...
    olAPI.getTranslation = function (tokens, localeKey) {
      ...
      while (index--) {
        if (!cache[tokens[index].msgName + ":" + tokens[index].msgCategory]) {
          whereSubClauses.push(
            "(Name='" +
              tokens[index].msgName +
              "' AND Category_vod__c='" +
              tokens[index].msgCategory +
              "')",
          );
        } else {
          cachedResponses.push(
            cache[tokens[index].msgName + ":" + tokens[index].msgCategory],
          );
        }
      }
      config.where +=
        "(" +
        whereSubClauses.join(" OR ") +
        ")" +
        " AND Language_vod__c='" +
        localeKey +
        "'";
      ...
      if (whereSubClauses.length) {
        olAPI
          ...
          .then(function (resp) {
            ...
            while (d--) {
              if (
                !cache[data[d].Name.value + ":" + data[d].Category_vod__c.value]
              ) {
                cache[
                  data[d].Name.value + ":" + data[d].Category_vod__c.value
                ] = data[d];
              }
            }
          });
    ...
    olAPI.queryVDSRecord = function (configObject) {
      ...
      if (typeof configObject === "object") {
        ...
        deferred.reject({
          success: false,
          message:
            "queryVDSRecord request failed: " + JSON.stringify(configObject),
        });
      }
    ...
    olAPI.getObjectMetadata = function (queryObject) {
      ...
      if (queryObject && queryObject.object) {
        ...
        queryObject.object === undefined
      ) {
        deferred.reject(
          onlineErrorResponse(
            errorCode.NO_PARAMETER,
            "getObjectMetadata called with no object: " +
        ...
        deferred.reject(
          onlineErrorResponse(
            errorCode.UNSUPPORTED_OBJECT,
            "getObjectMetadata called with an unsupported object: " +
              JSON.stringify(queryObject.object),
          ),
        );
      }
    ...
    olAPI.getObjectTypes = olAPI.getObjectTypeLabels = function (
      objectName,
      ...
      if (objectName) {
        ...
        deferred.reject(
          onlineErrorResponse(
            errorCode.NO_PARAMETER,
            "getObjectTypes called with no parameter",
          ),
        );
      }
...
(function (Q) {
  ...
  var DataService = function () {
    ...
    const ISO_DATE_REGEX =
      /[0-9]{4}-[0-9]{2}-[0-9]{2}(T|\s)[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{1,3}Z/;
    const VALID_DATE_FORMAT_REGEX =
      /[0-9]{4}.[0-9]{2}.[0-9]{2}(.[0-9]{2})?(.[0-9]{2})?(.[0-9]{2})?(.[0-9]{1,3})?[zZ]?/;
    ...
    function parseDate(input) {
      ...
      if (isISODateFormat(input)) {
        ...
        return new Date(
          extractedDigits.year,
          extractedDigits.month,
          extractedDigits.date,
          extractedDigits.hours,
          extractedDigits.minutes,
          extractedDigits.seconds,
          extractedDigits.ms,
        );
      }
    }
          ...
          for (var i = data.length; i--; ) {
            for (var d in data[i]) {
              if (data[i].hasOwnProperty(d)) {
                ...
                if (
                  cur_field &&
                  (cur_field.dataType === "date" ||
                    cur_field.dataType === "datetime")
                ) {
                  // date field
                ...
                } else if (cur_field && cur_field.dataType === "url") {
                  //url field
                  var tempUrl = data[i][d];
                  data[i][d] = {
                    value: tempUrl,
                    display:
                      tempUrl && tempUrl.length
                        ? '<a href="' +
                          tempUrl +
                          '" target="_blank">' +
                          tempUrl
                            .replace(urlTrimProtocolRE, "")
                            .replace(urlTrimDubRE, "")
                            .slice(0, 12) +
                          "\u2026</a>"
                        : null,
                    dataType: "url",
    ...
    function runQueryHelper(queryConfig, deferred) {
      var picklistsAvailable =
        queryConfig.picklists && queryConfig.picklists.length;
      ...
      function getPicklists(resp) {
        ...
        if (picklistsAvailable) {
          ...
          queryConfig.picklists.forEach(function (picklist) {
            var promise = ds
              .getPicklistValueLabels(queryObject, picklist, true)
                ...
                for (var pk in thisPicklistsResp) {
                  ...
                  if (thisPicklistsResp.hasOwnProperty(pk)) {
                    ...
                    for (
                      ...
                      if (
                        record[picklistName] &&
                        (picklistValues[record[picklistName].value] ||
                          picklistValues[record[picklistName]])
                      ) {
                        if (record[picklistName].value) {
                          record[picklistName].display =
                            picklistValues[record[picklistName].value];
      ...
      ds.getObjectMetadata({ object: queryConfig.object }).then(function (
        resp,
      ) {
      ...
      var allPromises = [
        objectLabelDeferred.promise,
        objectMetadataDeferred.promise,
        fieldLabelDeferred.promise,
        queryRecordDeferred.promise,
      ];
    ...
    function constructRequest(command, object, fields, where, sort, limit) {
      ...
      if (command && object) {
        ...
        console.error(
          "constructRequest: invalid arguments",
          command,
          object,
          fields,
        );
    ...
    function validateMethod(method) {
      return (
        typeof method === "string" && !!~methods.indexOf(method.toUpperCase())
      );
    }
    ...
    function validateExpect(expect) {
      return (
        ...
        (expect.toLowerCase() === "text" || expect.toLowerCase() === "blob")
      );
    }
      ...
      window[uniqueCallbackName] = function (resp) {
        ...
        if (typeof result === "string") {
          try {
            ...
            console.warn(
              "query result returned as non-parseable string",
              e,
              result,
            );
          }
    ...
    function postMessageRequest(messageId, messageBody) {
      ...
      if (MESSAGE_HANDLERS && MESSAGE_HANDLERS[messageId]) {
        ...
        MESSAGE_HANDLERS[messageId].postMessage(
    ...
    function formatDataEngineConfig(config) {
      ...
      if (
        config != null &&
        config.where != null &&
        config.where.type != null &&
        config.where.type.toUpperCase() !== "ALL_OF" &&
        config.where.type.toUpperCase() !== "ANY_OF"
      ) {
    ...
    function sendRequestAndRejectPromiseForFailure(
      request,
      deferred,
      resultKey,
    ) {
      ds.doPostMessage(request)
        .then((response) => {
    ...
    ds.joinQueries = function (
      ...
      for (d = secondaryQuery.data.length; d--; ) {
        for (k in secondaryQuery.data[d]) {
          if (secondaryQuery.data[d].hasOwnProperty(k) && k !== "date") {
            ...
            secondaryQuery.data[d][secondaryQuery.object.name + "." + k] =
              propertyToMove;
      ...
      for (d = joinTo.data.length; d--; ) {
        ...
        veevaUtil.mergeObjects(
          joinTo.data[d],
          joins[joinTo.data[d][keyForID].value],
        ); // does the key for ID include the object name or not. it will if joinTo === secondaryQuery;
      }
    ...
    ds.checkQueryQueue = function () {
      if (ds.queriesQueue.length && !ds.queryRunning) {
        ...
        if (next.type === "apiRequest") {
          ...
          next.type === "queryRecord" ||
          next.type === "queryRecord_ol"
        ) {
          ds.queryRecord(next.config).then(function (resp) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "querySalesData" ||
          next.type === "querySalesData_ol"
        ) {
          ds.querySalesData(next.config).then(function (resp) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "getAvailableObjects" ||
          next.type === "getAvailableObjects_ol"
        ) {
          ds.getAvailableObjects().then(function (resp) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "getObjectMetadata" ||
          next.type === "getObjectMetadata_ol"
        ) {
          ds.getObjectMetadata(next.config).then(function (resp) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "getObjectTypes" ||
          next.type === "getObjectTypes_ol"
        ) {
          ds.getObjectTypes(next.config, next.includeInactive).then(function (
            resp,
          ) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "getFieldLabels" ||
          next.type === "getFieldLabels_ol"
        ) {
          ds.getFieldLabels(next.config).then(function (resp) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "getObjectLabels" ||
          next.type === "getObjectLabels_ol"
        ) {
          ds.getObjectLabels(next.config).then(function (resp) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "getPicklistValueLabels" ||
          next.type === "getPicklistValueLabels_ol"
        ) {
          ds.getPicklistValueLabels(
            next.object,
            next.field,
            next.includeInactive,
          ).then(function (resp) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "getDataForCurrentObject" ||
          next.type === "getDataForCurrentObject_ol"
        ) {
          ds.getDataForCurrentObject(next.object, next.field).then(function (
            resp,
          ) {
            next.deferred.resolve(resp);
          }, genericQueryErrorHandler(next.deferred));
        } else if (
          next.type === "smartLinking" ||
          next.type === "smartLinking_ol"
        ) {
    ...
    ds.viewRecord = function (config) {
      ...
      if (config && config.object && config.fields) {
        ...
        ds.doPostMessage({ command: "viewRecord", configObject: config }).then(
          (resp) => handleViewResponse(resp, deferred),
          (resp) => handleViewResponse(resp, deferred),
        );
      } else {
        deferred.reject(
          createErrorResponse(
            errorCode.NO_PARAMETER,
            "viewRecord called with invalid configuration: " +
    ...
    function validTargetsField(targets, fields) {
      for (const target of targets) {
        ...
        for (const field of fields) {
          isValid =
            isValid || Object.prototype.hasOwnProperty.call(target, field);
        }
        if (!isValid) {
          return false;
        }
    ...
    ds.viewSection = function (target) {
      ...
      if (target) {
        ...
        if (validTargetsField(target, fields) && target.length > 0) {
          ...
          deferred.reject(
            createErrorResponse(
              ...
              "viewSection called with invalid parameter target : " +
        ...
        deferred.reject(
          createErrorResponse(
            errorCode.NO_PARAMETER,
            "viewSection called with no target : " + JSON.stringify(target),
          ),
        );
      }
      return deferred.promise;
    ...
    ds.createRecord = function (config) {
      ...
      if (config && config.object) {
        ...
        deferred.reject(
          createErrorResponse(
            errorCode.GENERIC,
            "createRecord called with invalid configuration: " +
              JSON.stringify(config),
          ),
        );
      }
    ...
    ds.updateRecord = function (config) {
      ...
      if (config && config.object) {
        ...
        deferred.reject(
          createErrorResponse(
            errorCode.GENERIC,
            "updateRecord called with invalid configuration: " +
              JSON.stringify(config),
          ),
        );
      }
    ...
    ds.request = function (requestObject) {
      ...
      if (errorMessage !== "Valid") {
        deferred.reject(
          createErrorResponse(errorCode.INVALID_PARAM, errorMessage),
        );
      } else {
    ...
    ds.executeSuggestionAction = function (
      suggestionId,
      actionType,
      bulkExecution,
    ) {
    ...
    ds.launchMediaForAccount = function (
      accountId,
      presentationId,
    ...
    ds.getObjectTypes = ds.getObjectTypeLabels = function (
      objectName,
      includeInactive = false,
    ) {
      var deferred = Q.defer();
      ...
      if (slideIds && Array.isArray(slideIds) && slideIds.length) {
        ...
        if (slideIds.some((id) => typeof id !== "string")) {
          deferred.reject(
            createErrorResponse(
              errorCode.INVALID_PARAM,
              "getMediaImagesForSlides called with invalid configuration: " +
      ...
      if (
        ...
        if (presentationIds.some((id) => typeof id !== "string")) {
          deferred.reject(
            createErrorResponse(
              errorCode.INVALID_PARAM,
              "getFirstSlideForPresentations called with invalid configuration: " +
                JSON.stringify(presentationIds),
            ),
          );
        } else {
          return ds.doPostMessage(messageConfig);
    ...
    ds.getMediaImagesForPresentations = function (presentationIds) {
      ...
      if (
        ...
        if (presentationIds.some((id) => typeof id !== "string")) {
          ...
          return ds
            .getFirstSlideForPresentations(presentationIds)
            .then(async (response) => {
              ...
              if (slideIds.length) {
                ...
                if (
                  ...
                  for (const slideId in imageResponse.data) {
                    result[slideToPresentationMap[slideId]] =
                      imageResponse.data[slideId];
                  }
                } else {
                  return deferred.promise;
                }
        ...
        deferred.reject(
          createErrorResponse(
            errorCode.INVALID_PARAM,
            "getMediaImagesForPresentations called with invalid configuration: " +
              JSON.stringify(presentationIds),
          ),
        );
      }
    ...
    ds.queryDataEngine = function (configObject) {
      ...
      if (configObject && configObject.table) {
        ...
        for (const prop in configObject) {
          if (!validProps.includes(prop.toLowerCase())) {
            ...
            deferred.reject(
              createErrorResponse(
                errorCode.INVALID_PARAM,
                "queryDataEngine called with invalid parameter " +
                  prop +
                  ": " +
        ...
        deferred.reject(
          createErrorResponse(
            errorCode.NO_PARAMETER,
            "queryDataEngine called with no table: " +
              JSON.stringify(configObject),
          ),
        );
      }
    ...
    ds.getMessages = function (tokens) {
      ...
      if (tokens && tokens instanceof Array) {
          ...
          for (const prop in token) {
            if (!validProps.includes(prop)) {
              ...
              deferred.reject(
                createErrorResponse(
                  errorCode.INVALID_PARAM,
                  "getMessages called with invalid token property - " + prop,
                ),
              );
              break;
            }
        ...
        deferred.reject(
          createErrorResponse(
            errorCode.NO_PARAMETER,
            "getMessages called without a valid array of tokens: " +
              JSON.stringify(configObject),
          ),
        );
      }
    ...
    ds.getVeevaMessagesWithDefault = function (tokens) {
      ...
      return ds.getMessages(getMessagesTokens).then((resp) => {
        return resp.map((msg) => {
          return {
            Category_vod__c: {
              value: msg.group,
    ...
    ds.getDataEngineTableMetadata = function (tableName) {
      ...
      if (tableName) {
        ...
        deferred.reject(
          createErrorResponse(
            errorCode.NO_PARAMETER,
            "getDataEngineTableMetadata called with no tableName: " +
